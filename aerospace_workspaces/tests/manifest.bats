...
@test "manifest declares every wire op the script emits" {
  # Wire ops the script uses: add/set/remove/move (ridge add|set|remove|move),
  # bracket (ridge bracket ...), query (ridge query items),
  # brackets (ridge query brackets - a DISTINCT wire op from query).
  for op in add set remove move bracket query brackets; do
...
@test "script emits no ridge verb outside the declared set" {
  # Every `ridge <verb>` in the script/emitted commands must be one of the
  # declared verbs (query subcommands map to query/brackets ops).
  run bash -c "grep -ohE 'ridge (add|set|remove|move|refresh|query|subscribe|popup|bracket) ?' \
    '${PLUGIN_DIR}/aerospace-workspaces-plugin.sh' | awk '{print \$2}' | sort -u"
  [ -n "$output" ] || { echo "no ridge verbs extracted - check the grep pattern"; false; }
  for verb in $output; do
    case "$verb" in
      add|set|remove|move|bracket|query) : ;;
