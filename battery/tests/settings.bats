...
@test "load_settings applies defaults" {
  ...
  [ "$SETTING_warn_threshold" = "20" ]
  [ "$SETTING_crit_threshold" = "10" ]
  [ "$SETTING_warn_color" = "theme:warning" ]
  [ "$SETTING_crit_color" = "theme:error" ]
  [ "$SETTING_normal_color" = "theme:primary" ]
...
@test "load_settings overrides from the settings JSON" {
  local f; f="$(mktemp)"
  printf '%s' '{"region":"left","warn_threshold":"30","normal_color":"#123456"}' > "$f"
  RIDGE_PLUGIN_SETTINGS="$f" load_settings
  [ "$SETTING_region" = "left" ]
  [ "$SETTING_warn_threshold" = "30" ]
  [ "$SETTING_normal_color" = "#123456" ]
  [ "$SETTING_crit_threshold" = "10" ]   # untouched key keeps default
  rm -f "$f"
}
...
@test "load_settings rejects zero-equivalent interval values" {
  ...
  for v in "0" "00" "0.0" "0.00" ".0"; do
    ...
    RIDGE_PLUGIN_SETTINGS="$f" load_settings
    [ "$SETTING_interval" = "120" ] || { echo "interval=$v not rejected: got $SETTING_interval"; rm -f "$f"; return 1; }
    rm -f "$f"
  done
}
