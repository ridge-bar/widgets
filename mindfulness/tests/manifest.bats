...
@test "manifest declares every wire op the script emits" {
  # Wire ops the script uses: add/set/remove (ridge add|set|remove), popup
  # (ridge popup toggle|show|set-rows all map to the single "popup" op), and
  # queryPopup (ridge query popup <id>, TASK-148 - distinct from the plain
  # "query" op since each `query` subject is its own Op - see
  # Sources/RidgeCore/APIProtocol.swift's Op enum).
  for op in add set remove popup queryPopup; do
...
@test "script emits no ridge verb outside the declared set" {
  run bash -c "grep -ohE 'ridge (add|set|remove|move|refresh|query|subscribe|popup|bracket) ?' \
    '${PLUGIN_DIR}/mindfulness.sh' | awk '{print \$2}' | sort -u"
  [ -n "$output" ] || { echo "no ridge verbs extracted - check the grep pattern"; false; }
  for verb in $output; do
    case "$verb" in
      add|set|remove|popup|query) : ;;
      *) echo "script uses undeclared verb: $verb"; false ;;
    esac
  done
}

@test "the script's only query use is 'ridge query popup', mapping to the queryPopup op" {
  run bash -c "grep -ohE 'ridge query [a-z-]+' '${PLUGIN_DIR}/mindfulness.sh' | sort -u"
  [ "$output" = "ridge query popup" ]
}
...
@test "plugin.yaml is valid YAML with owns=mindfulness." {
